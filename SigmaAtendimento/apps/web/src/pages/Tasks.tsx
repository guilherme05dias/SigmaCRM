...
function TaskRow({ task, onOpen, onComplete }: { task: Task; onOpen: () => void; onComplete: () => void }) {
    ...
    return (
        <div className="group grid min-h-[58px] grid-cols-[32px_minmax(0,1fr)] items-center border-b border-border px-2 last:border-b-0 hover:bg-surface-alt/60 md:grid-cols-[40px_minmax(240px,1fr)_120px_110px_130px_44px]">
...
export default function Tasks() {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground md:flex">
            ...
            <main className="min-w-0 flex-1 pb-28 pt-[env(safe-area-inset-top)] md:pb-8 md:pt-0">
                <div className="mx-auto w-full max-w-[1600px] px-3 py-5 sm:px-4 sm:py-6 md:px-8 md:py-8">
                    ...
                    <section className="mt-5 rounded-2xl border border-border bg-surface">
                        <div className="flex flex-col gap-3 border-b border-border p-3 lg:flex-row lg:items-center lg:justify-between">
                            <div className="flex min-w-0 flex-wrap gap-2">
                                <div className="grid w-full min-w-0 grid-cols-3 rounded-xl bg-surface-alt p-1 sm:flex sm:w-auto">{([['list', 'task_list', 'Lista'], ['board', 'view_kanban', 'Quadro'], ['calendar', 'calendar_month', 'Agenda']] as const).map(([mode, icon, label]) => <button key={mode} type="button" onClick={() => setView(mode)} className={`flex min-h-10 min-w-0 items-center justify-center gap-1 rounded-lg px-1.5 text-xs font-semibold sm:gap-2 sm:px-3 sm:text-sm ${view === mode ? 'bg-surface text-primary shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}><Icon name={icon} className="size-4 shrink-0" />{label}</button>)}</div>
                            ...
                            <div className="grid min-w-0 flex-1 grid-cols-2 gap-2 lg:flex lg:flex-wrap lg:items-center lg:justify-end">
                                <label className="relative col-span-2 min-w-0 lg:min-w-[220px] lg:flex-1 lg:max-w-sm"><Icon name="search" className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar tarefa, cliente ou chamado" className="h-10 w-full min-w-0 rounded-xl border border-border bg-surface pl-9 pr-3 text-sm text-foreground outline-none focus:border-primary" /></label>
                                <select value={priority} onChange={(event) => setPriority(event.target.value as Priority | '')} className="h-10 min-w-0 w-full rounded-xl border border-border bg-surface px-2 text-sm text-foreground lg:w-auto lg:px-3"><option value="">Prioridades</option><option value="CRITICAL">Crítica</option><option value="HIGH">Alta</option><option value="MEDIUM">Média</option><option value="LOW">Baixa</option></select>
                                <select value={source} onChange={(event) => setSource(event.target.value as TaskSource | '')} className="h-10 min-w-0 w-full rounded-xl border border-border bg-surface px-2 text-sm text-foreground lg:w-auto lg:px-3"><option value="">Origens</option>{Object.entries(sourceConfig).map(([value, config]) => <option key={value} value={value}>{config.label}</option>)}</select>
                                <select value={serviceTopicFilter} onChange={(event) => setServiceTopicFilter(event.target.value)} className="col-span-2 h-10 min-w-0 w-full rounded-xl border border-border bg-surface px-2 text-sm text-foreground lg:col-span-1 lg:w-auto lg:px-3"><option value="">Sistemas/produtos</option>{serviceTopics.filter((topic) => topic.active !== false).map((topic) => <option key={topic.id} value={topic.id}>{topic.name}</option>)}</select>
                                {canManage && scope === 'team' && <select value={assignee} onChange={(event) => setAssignee(event.target.value)} className="col-span-2 h-10 min-w-0 w-full rounded-xl border border-border bg-surface px-2 text-sm text-foreground lg:col-span-1 lg:w-auto lg:px-3"><option value="">Toda a equipe</option>{users.filter((item) => item.active !== false).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>}
                            </div>
                        </div>
